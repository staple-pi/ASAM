...
def train_one_epoch(asam_model,sam_o, d_model, train_dataloader,epoch,optimizer, optimizer_d, device,batch_size):
    ...
    for step, (input_image, input_image_o, bbox_torch, gt_mask, maskin,v_64,occ_64) in enumerate(train_dataloader):
        ...
        v_64, occ_64, maskin = v_64.to(device), occ_64.to(device), maskin.to(device)
        ...
        '''
        v_feature = image_feature * v_64
        o_feature = image_feature * occ_64
        input_flat = o_feature.view(256, -1)    # [256, 4096]
        target_flat = v_feature.view(256, -1)  # [256, 4096]
        input_log_prob = F.log_softmax(input_flat, dim=1)  # [256, 4096]
        target_prob = F.softmax(target_flat, dim=1)        # [256, 4096]
        kl_loss = F.kl_div(input_log_prob, target_prob, reduction='batchmean')  
        '''
        loss1 = seg_loss(asam_pred, gt_mask) 
        loss2 = 5 * ce_loss(asam_pred, gt_mask.float())
        ...
        g_loss = 0.5 *loss_d(d_model(image_o,asam_pred), torch.zeros(size=(batch_size,1),device=device,requires_grad=True))
