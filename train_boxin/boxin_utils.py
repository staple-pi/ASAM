...
def train_one_epoch(asam_model,sam_o, d_model, train_dataloader,epoch,optimizer, optimizer_d, device,batch_size,savepath,writer):
    ...
    for step, (input_image, input_image_o, bbox_torch, gt_mask, v_mask,omask,maskin,image_filepath) in enumerate(train_dataloader):
        optimizer.zero_grad()
        image, image_o, bbox = input_image.to(device),input_image_o.to(device), bbox_torch.to(device)
        gt_mask, v_mask, omask, maskin = gt_mask.to(device), v_mask.to(device), omask.to(device), maskin.to(device)
