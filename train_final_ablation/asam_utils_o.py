...
def train_ablation1(asam_model, d_model, train_dataloader,epoch,optimizer, optimizer_d, device,batch_size,writer):  ###################
    ...
    for step, (input_image,input_image_o, bbox_torch, gt_mask, v_mask, maskin_torch, v_64, o_64) in enumerate(train_dataloader):
        ...
        asam_pred_s = torch.sigmoid(asam_pred)
        g_loss = loss_d(d_model(image_o,asam_pred_s), torch.zeros(size=(batch_size,1),device=device,requires_grad=True))
        loss = dloss + bloss0 + bloss1 + g_loss
        ...
        if is_main_process():
            train_dataloader.desc = "[epoch {}] mean loss {},dloss {},bloss0 {},bloss1 {},gloss {},diloss {}".format(epoch, round(mean_loss.item(), 4),
                                    round(mean_dloss.item(), 4),round(mean_bloss0.item(), 4),round(mean_bloss1.item(), 4),
...
def train_ablation4(asam_model, train_dataloader,epoch,optimizer, device,batch_size,writer):  ###################
    ...
    for step, (input_image,input_image_o, bbox_torch, gt_mask, v_mask, maskin_torch, v_64, o_64) in enumerate(train_dataloader):
        ...
        if is_main_process():
            train_dataloader.desc = "[epoch {}] mean loss {},dloss {},bloss0 {},bloss1 {}".format(epoch, round(mean_loss.item(), 4),
                                    round(mean_dloss.item(), 4),round(mean_bloss0.item(), 4),round(mean_bloss1.item(), 4))        
